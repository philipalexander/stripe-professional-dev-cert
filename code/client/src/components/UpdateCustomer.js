...
const UpdateCustomer = ({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const setupIntent = await fetch(`/update-payment-details/${customerId}`, {
        method: "POST",
        mode: 'cors',
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body)
      })
      const json_res = await setupIntent.json();
      if (json_res.error) {
        setError(json_res.error)
        setProcessing(false);
        return;
      }
      setStripeOptions({clientSecret: json_res.client_secret})
